                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($discounts as $discount)
                        <tr>
                            ...
                            <td>
                                <button type="button" onclick="window.location.href='{{ route('discount.edit', $discount->id) }}'"  class="text-white text-m bg-[#F7BE38] hover:bg-[#F7BE38]/90 focus:ring-4 focus:outline-none focus:ring-[#F7BE38]/50 font-medium rounded-lg px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#F7BE38]/50 me-2 mb-2">
                                    Edit
                                </button>

                                <form id="delete-form-{{ $discount->id }}" action="{{ route('discount.destroy', $discount->id) }}" method="POST" class="inline-block ml-2">
                                    @csrf
                                    @method('DELETE')
