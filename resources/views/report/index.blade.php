...
@section('content')
    <div class="max-w-6xl px-4 mx-auto my-8">
        ...
        @if ($type === 'daily')
            <form method="GET" action="{{ route('report.index') }}" class="mb-4">
                ...
                <button type="submit" class="btn btn-primary ml-2 bg-yellow-300">Filter</button>
            </form>
        @elseif ($type === 'monthly')
            <form method="GET" action="{{ route('report.index') }}" class="mb-4">
        ...
        <!-- Tabel laporan -->
        <table class="table-auto font-inter w-full border-collapse border border-gray-400">
            <thead>
            ...
            <tbody>
                ...
                @endforelse
                <tr class="font-bold">
                    <th class="border border-gray-400 px-4 py-2" colspan="2">Total</th>
                    <th class="border text-start border-gray-400 px-4 py-2">Rp
                        {{ number_format($salesReport->sum('total_price'), 0, ',', '.') }}</th>
                </tr>
            </tbody>
        </table>
    </div>
