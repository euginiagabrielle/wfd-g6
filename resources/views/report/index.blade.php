...
@section('content')
    ...
    <div class="max-w-6xl mx-auto my-8 px-8">
        ...
        @if ($type === 'daily')
            <form method="GET" action="{{ route('report.index') }}" class="mb-4">
                ...
                <input type="date" name="date" value="{{ request('date') }}"
                    class="p-2 border rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent">
                <button type="submit"
                    class="text-white ml-2 bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-m px-5 py-2.5 text-center me-2 mb-2">
                    FILTER
                </button>
            </form>
        @elseif ($type === 'monthly')
        ...
        @endif
        <div class="mx-auto pb-3 w-full">
            <div class="bg-white rounded-xl p-4 font-inter ">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($salesReport as $report)
                            <tr>
                                ...
                                <td class="border border-gray-400 px-4 py-2">Rp
                                    {{ number_format($report->total_price, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="border border-gray-400 px-4 py-2 text-center">No data
                                    available.</td>
                            </tr>
                        @endforelse
